...
// IPv6 literals) by literal hostname/IP at monitor create/update time.
//
...
//      create-time validation but a private IP at actual probe-time would
//      bypass this entirely (a complete guard would need to resolve DNS
//      and re-check on every single probe, not just at create/update).
//   2. Redirects: a probe to an allowed public URL that 302s to a blocked
//      internal target at request time - native fetch() follows redirects
//      by default and this guard only ever runs once, at create/update.
// Both are out of scope for this module's "minimal implementation"
// guidance. Only authenticated MONITOR_MANAGE users can set a target URL
// at all, which bounds who could attempt this, but that's a weaker
// guarantee than not letting the literal-obvious cases through at all.
//
// What IS covered (security review finding, Module 8): IPv6 was
// originally unhandled entirely - a bracketed IPv6 loopback/link-local/
// unique-local literal, or an IPv4-mapped IPv6 literal encoding the exact
// cloud-metadata address (e.g. ::ffff:169.254.169.254, which Node's own
// URL parser normalizes to the hex form ::ffff:a9fe:a9fe), sailed straight
// through both the hostname allowlist and the IPv4-only regex. Both forms
// are now checked.
