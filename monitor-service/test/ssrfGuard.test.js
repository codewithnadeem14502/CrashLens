...
// The uptime prober performs outbound HTTP requests to a user-supplied URL
// (only an authenticated MONITOR_MANAGE user can set it, but a compromised
// or malicious one could otherwise use monitor-service as an SSRF proxy
// into the Docker network's internal services - Mongo, Redis, RabbitMQ,
// other backend services, or a cloud metadata endpoint).
...
// Security review finding (Module 8): IPv6 was originally unhandled
// entirely - these are concrete bypasses that were confirmed to sail
// through the pre-fix guard, including the exact cloud-metadata address
// via its IPv4-mapped IPv6 encoding.
...
test("rejects the cloud-metadata address via its IPv4-mapped IPv6 encoding, both dotted and hex forms", () => {
  ...
  // Node's own URL parser normalizes the dotted form above to this hex
  // form - both must be caught since a caller could construct either.
